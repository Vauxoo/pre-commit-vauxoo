...
from pre_commit_vauxoo.hooks import check_semgrep_odoo
...
class TestPreCommitVauxoo:
    ...
    def test_semgrep_entry_point_reports_without_failing(self, capfd):
        """The entry point itself, not a reimplementation of its flags

        It must report the findings and still exit zero, because the check is
        experimental and may not decide a build.
        """
        if not shutil.which("semgrep"):
            pytest.skip("semgrep is not installed")
        self.runner.invoke(main, ["--only-cp-cfg"])
        ...
        assert check_semgrep_odoo.main([str(fixture)]) == 0
        output = capfd.readouterr()
        assert "odoo-bare-determinant-compute" in output.out
        # semgrep says it cannot install a segfault handler on macOS on every run,
        # which is noise from its runtime and must not reach the report
        assert "Failed to register" not in output.err

    def test_semgrep_entry_point_skips_without_a_config(self, capfd):
        """A missing config must not fail the run, it is generated by this same tool"""
        # setup_method already left the working directory at self.tmp_dir, which has
        # no .config because this test never generates one
        assert check_semgrep_odoo.main(["anything.py"]) == 0
        assert "Skipping the semgrep checks" in capfd.readouterr().out
